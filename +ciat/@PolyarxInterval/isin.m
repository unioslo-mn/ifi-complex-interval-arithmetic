function r = isin(obj,x)
    ...
    if Mo*No == 1
        r = inPolyarc([obj.Arcs{:};obj.Vertices{:}],x);
    elseif Mx*Nx == 1
        r(Mo,No) = false;
        for m = 1:Mo
            for n = 1:No
                r(m,n) = inPolyarc([obj.Arcs{m,n};obj.Vertices{m,n}],x);
            end
        end
    elseif No*Mx == 1
        r(Mo,Nx) = false;
        for m = 1:Mo
            r(m,:) = inPolyarc([obj.Arcs{m};obj.Vertices{m}],x);
        end
    elseif Mo*Nx == 1
        r(Mx,No) = false;
        for n = 1:No
            r(:,n) = inPolyarc([obj.Arcs{n};obj.Vertices{n}],x.').';
        end
    else
